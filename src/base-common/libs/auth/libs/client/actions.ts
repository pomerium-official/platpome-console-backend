...
import {createLoginUrl, logoutUrl} from '../shared/oidc-urls';
import {ActionSaveIdTokenOther, clientId, redirectUrl,} from '../shared/shared-config';
import {verifyJwt} from '../client/verify-jwt';
import jwtDecode from 'jwt-decode';
import {UserInfo} from '../client/model';
...
export const logout = async () => {
  ...
  // 동일 클라이언트인 경우 로그아웃 키클록에 처리
  if (idToken) {
    const idInfo = jwtDecode<UserInfo>(idToken);

    if (idInfo.aud === clientId) {
      const logoutAction =
        logoutUrl +
        '?client_id=' +
